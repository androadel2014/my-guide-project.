...
export const CVBuilderView = ({ lang }) => {
  ...
  // 2. دالة جلب الـ CV المحفوظ من السيرفر (تشتغل مرة واحدة عند التحميل)
  useEffect(() => {
    if (currentUser && currentUser.id) {
      fetch(`http://localhost:5000/api/get-cv/${currentUser.id}`)
        .then((res) => res.json())
        .then((savedData) => {
          // لو لقينا بيانات قديمة في الداتابيز، بنحملها
          if (savedData && !savedData.message) {
            setData(savedData);
          } else {
            // لو مستخدم جديد ومفيش CV قديم، بنملى اسمه وإيميله من بيانات التسجيل كبداية
            setData((prev) => ({
              ...prev,
              personalInfo: {
                ...prev.personalInfo,
                fullName:
                  prev.personalInfo.fullName || currentUser.username || "",
                email: prev.personalInfo.email || currentUser.email || "",
              },
            }));
          }
        })
        .catch((err) => console.log("مستخدم جديد أو السيرفر مغلق"));
    }
  }, [currentUser]); // تعتمد على وجود المستخدم

  // 3. نظام الحفظ التلقائي الذكي (Auto-Save)
  useEffect(() => {
    if (currentUser && currentUser.id) {
      // بنعمل تايمر يبعت للسيرفر بعد ثانيتين من توقف الكتابة
      const delaySave = setTimeout(() => {
        fetch("http://localhost:5000/api/save-cv", {
          ...
          body: JSON.stringify({
            ...
            cv_data: data, // البيانات الحالية اللي اليوزر شغال عليها
          }),
        })
          .then((res) => res.json())
          .then((resData) => console.log("✅ تم الحفظ التلقائي في الداتابيز"))
          .catch((err) => console.error("❌ فشل الحفظ التلقائي:", err));
      }, 2000);

      // تنظيف التايمر لو اليوزر كتب حرف جديد قبل ما الـ 2 ثانية تخلص
      return () => clearTimeout(delaySave);
    }
  }, [data, currentUser]); // يشتغل كل ما الـ data تتغير
  ...
  const removeItem = (section, id) =>
    setData({ ...data, [section]: data[section].filter((i) => i.id !== id) });
  ...
  const addHomeCountryExp = () => {
    ...
    setData({ ...data, experience: [...data.experience, mockExp] });
  };
          ...
          <div className="mb-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <input
                ...
                onBlur={(e) => addToHistory("addresses", e.target.value)}
              />
            </div>
          </div>
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-3 mb-3">
                  <input
                    ...
                    value={job.title}
                    onChange={(e) =>
                      updateItem("experience", job.id, "title", e.target.value)
                    }
                    onBlur={(e) => addToHistory("jobTitles", e.target.value)}
                  />
                  <input
                  ...
                  <div className="flex gap-2">
                    ...
                    <input
                      ...
                      value={job.end}
                      onChange={(e) =>
                        updateItem("experience", job.id, "end", e.target.value)
                      }
                    />
                  </div>
                </div>
