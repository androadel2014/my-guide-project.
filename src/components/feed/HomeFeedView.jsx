...
/* =========================
   ...
   - View replies toggle
========================= */
const VISUAL_MAX_DEPTH = 2; // أقصى عمق بصري
const INDENT_PX = 16; // مسافة خفيفة للردود
...
const CommentNode = ({
  ...
  const replyKey = `${postId}:${node.id}`; // input open/close
  const threadKey = `thread:${postId}:${node.id}`; // view replies open/close
  ...
  return (
    ...
    >
      {/* left line for nested replies (facebook style) */}
      {cappedDepth > 0 && (
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex items-center gap-4 text-xs text-gray-500">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onToggleReply(postId, node.id); // open reply input
              }}
            ...
            {hasReplies && (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  onToggleReply(postId, threadKey); // open/close replies
                }}
...
export const HomeFeedView = () => {
  ...
  // ✅ toggle reply input OR toggle thread replies
