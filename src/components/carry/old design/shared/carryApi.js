...
/* =====================
   MATCH REQUEST (Shipment -> Trip)
===================== */
// ✅ Now requires: shipment_id + offer_amount + offer_currency + note
export async function requestCarryMatch(
  listingId,
  { shipment_id, offer_amount, offer_currency = "USD", note } = {}
) {
  ...
  if (r?.ok) {
    const status =
      ...
      r?.data?.status ||
      extractStatusFromRequestResponse(r);
